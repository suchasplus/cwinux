...
void CwxMd5::update(unsigned char *szBuf, CWX_UINT32 uiLen)
{
    ...
    /* Update bitcount */
    t = m_bits[0];
    if ((m_bits[0] = t + ((CWX_UINT32) uiLen << 3)) < t)
        m_bits[1]++;
    m_bits[1] += uiLen >> 29;
    ...
    if (t)
    {
        ...
        if (uiLen < t)
        {
            memcpy(p, szBuf, uiLen);
            return;
        }
        memcpy(p, szBuf, t);
        byteReverse(m_in, 16);
        transform(m_buf, (CWX_UINT32 *) m_in);
        szBuf += t;
        uiLen -= t;
    }
...
void CwxMd5::final(unsigned char digest[16])
{
    ...
    *p++ = 0x80;
    ...
    ((CWX_UINT32 *) m_in)[14] = m_bits[0];
    ((CWX_UINT32 *) m_in)[15] = m_bits[1];
...
*/
void CwxMd5::transform(CWX_UINT32 buf[4], CWX_UINT32 const in[16]){
    register CWX_UINT32 a, b, c, d;
    a = buf[0];
    b = buf[1];
    c = buf[2];
    d = buf[3];
