...
void CwxMd5::md5_process(unsigned char data[64])
{
...
void CwxMd5::update(unsigned char const* input, CWX_UINT32 length)
{
    uint32_t left, fill;
    ...
    if (left && length >= fill) {
        memcpy((void *) (buffer + left), (void *) input_1, fill);
        md5_process(buffer);
        length -= fill;
        input_1 += fill;
        left = 0;
    ...
    while (length >= 64) {
        md5_process((unsigned char *)input_1);
        length -= 64;
        input_1 += 64;
    }

    if (length) {
        memcpy((buffer + left), input_1, length);
    }
}
