...
CwxKeyValueItemEx const* CwxPackageReaderEx::getKey(char const* szKey, bool bSubKey) const
{
    ...
    if (!bSubKey || !pNextSub){
        ...
        pNextSub++;
        CwxKeyValueItemEx const* pItem = this->getKey(key.c_str(), false);
        if (!pItem) return NULL;
        if (!pItem->m_bKeyValue) return NULL;
        CwxPackageReaderEx reader(m_bCaseSensive);
