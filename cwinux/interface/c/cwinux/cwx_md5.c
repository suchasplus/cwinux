/* $XJG: xslib/md5.c,v 1.6 2006/03/16 04:25:13 jiagui Exp $ */
/*
...
#ifdef XSLIB_RCSID
static const char rcsid[] = "$XJG: xslib/md5.c,v 1.6 2006/03/16 04:25:13 jiagui Exp $";
#endif
...
void md5_update(md5_context * ctx, const void *input, unsigned int length)
{
	...
	if (left && length >= fill) {
		memcpy((void *) (ctx->buffer + left), (void *) input_1, fill);
	...
	while (length >= 64) {
		md5_process(ctx, (unsigned char *)input_1);
		length -= 64;
		input_1 += 64;
	}

	if (length) {
		memcpy((ctx->buffer + left), input_1, length);
	}
}
